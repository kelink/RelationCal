...
public class ScheduledThreadPoolTest {
	...
	public static void beepForTime() {
		 ...
		 * 后执行，依此类推。如果任务的任何一个执行遇到异常， 则后续执行都会被取消。否则，只能通过执行程序的取消或终止方法来终止该任务。
		 * 如果此任务的任何一个执行要花费比其周期更长的时间，则将推迟后续执行，但不会同时执行。 参数： command - 要执行的任务
		 * initialDelay - 首次执行的延迟时间 period - 连续执行之间的周期 unit - initialDelay 和
		 * period 参数的时间单位
		 * 
		 */
		// final ScheduledFuture<?> beeperHandle =
		// scheduler.scheduleAtFixedRate(beeper, startTime, each,
		// TimeUnit.SECONDS);
		 ...
		 * 如果任务的任一执行遇到异常，就会取消后续执行。 否则，只能通过执行程序的取消或终止方法来终止该任务。 参数： command -
		 * 要执行的任务 initialDelay - 首次执行的延迟时间 period - 连续执行之间的周期 unit -
		 * initialDelay 和 period 参数的时间单位
		 * 
		 */
		final ScheduledFuture<?> beeperHandle = scheduler
				.scheduleWithFixedDelay(new CalTask(), startTime, each,
						TimeUnit.SECONDS);
